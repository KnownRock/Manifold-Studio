...
import type { Manifold, Mesh } from '../public/manifold-encapsulated-types.js';
...
interface WorkerStatic extends ManifoldToplevel {
  Manifold: String | OffscreenCanvas | Manifold;
...
async function evaluateCADToManifold(code: string) {
  ...
  console.log({ globalDefaults, manifold })

  return { globalDefaults, manifold };
}

export function getManifoldRuningContext() {
  return {
    globalDefaults,
    exportModels,
    glMatrix,
