...
import {Accessor, Animation, AnimationSampler, Document,type mat4, Material, Mesh as GLTFMesh, Node, WebIO} from '@gltf-transform/core';
...
import type {CrossSection, Manifold, ManifoldToplevel, Mesh, Vec3} from '../public/manifold-encapsulated-types.js'
import {type Properties, setupIO, writeMesh} from './gltf-io';
import type {GLTFMaterial, Quat} from '../public/editor';
import type { Manifold } from '../public/manifold-encapsulated-types.js';
...
async function evaluateCADToManifold(code: string) {
  ...
  const manifold = await evalFn(...Object.values(context))()
...
export async function evaluateCADToModel(code: string) {
  ...
  const {globalDefaults, manifold} = await evaluateCADToManifold(code);
  const t1 = performance.now();
  const models = await exportModels(globalDefaults, manifold);
  const t2 = performance.now();

  console.log(`Evaluation took ${t1 - t0} ms`);
  console.log(`Exporting took ${t2 - t1} ms`);
